        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
                Monitoring: {selectedMachine || "Loading..."} ({chartData.length} data points)
            </p>
          </div>
        </div>
        ...
        <div className="h-[350px] w-full">
          {loadingSensor ? (
             <div className="h-full w-full flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 gap-2">
               ...
               <p className="text-sm">Fetching sensor data...</p>
             </div>
          ) : error ? (
             <div className="h-full w-full flex flex-col items-center justify-center text-red-500 dark:text-red-400 gap-2">
               <p className="text-sm">Error loading sensor data</p>
               <p className="text-xs">{error instanceof Error ? error.message : 'Unknown error'}</p>
             </div>
          ) : !chartData || chartData.length === 0 ? (
             <div className="h-full w-full flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 gap-2">
               <p className="text-sm">No sensor data available for {selectedMachine}</p>
               <p className="text-xs">Start simulation to generate data</p>
             </div>
          ) : (
